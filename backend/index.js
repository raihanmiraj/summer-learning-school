...
let uri = `mongodb+srv://${process.env.DB_NAME}:${process.env.DB_PASS}@cluster0.dhnvk0f.mongodb.net/?retryWrites=true&w=majority`;
let users = []; 
...
async function run() {
  try {
    ...
    app.get("/class/:classid",verifyJWT, async (req, res) => {
...
const result2 = await classes.updateOne(filter2, updateDoc2);
    ...
    app.get('/addnow',verifyJWT, async(req, res) =>{
      const pipeline =[
    ...
    app.get("/classes/find/:email",verifyJWT, async (req, res) => {
    ...
    app.get("/admin/classes/all",verifyJWT, async (req, res) => { 
       let cursor = classes.find();
      let result = await cursor.toArray();
      res.send(result);

    });
    app.get("/instructor/all",verifyJWT, async (req, res) => { 
