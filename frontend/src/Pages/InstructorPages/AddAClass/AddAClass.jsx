...
const AddAClass = ({setTitle}) => {
    ...
    const  { user, toastPush,userData, setUserData}  = useContext(AuthContext);
     useEffect(() => {
      setTitle("Add Class")
        setclassesData({...classesData, instructorname:user && user.displayName?user.displayName:userData.name, instructoremail:user && user.email?user.email:userData.email})
        setLoading(false)
      }, []);
